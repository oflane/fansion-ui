...
<template>
  <div class="fac-form-group" >
    <el-form ref="elForm" v-for="(group,$index) in groups" :status-icon="statusIcon" :model="model" :key="$index" class="fac-form" :class="groupCss(group)" :label-width="labelW" :label-position="labelPos">
...
<script>
   ...
   * 获取工具方法
   */
  const {isNotNull, noneValue} = fase.util
  ...
  export default {
    ...
    data: function () {
      const labelW = noneValue(this.conf.labelWidth, this.labelWidth, '120px')
      const labelPos = noneValue(this.conf.labelPosition , this.labelPosition , 'right')
      const gutterProp = noneValue(this.conf.gutter , this.gutter , 20)
